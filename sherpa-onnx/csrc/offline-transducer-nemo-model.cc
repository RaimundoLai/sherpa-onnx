 ...
 public:
  explicit Impl(const OfflineModelConfig &config)
    ...
    encoder_sess_ = std::make_unique<Ort::Session>(
        GetOrtEnv(), SHERPA_ONNX_TO_ORT_PATH(config.transducer.encoder_filename),
        sess_opts_);
    InitEncoder(nullptr, 0);

    decoder_sess_ = std::make_unique<Ort::Session>(
        GetOrtEnv(), SHERPA_ONNX_TO_ORT_PATH(config.transducer.decoder_filename),
        sess_opts_);
    InitDecoder(nullptr, 0);

    joiner_sess_ = std::make_unique<Ort::Session>(
        GetOrtEnv(), SHERPA_ONNX_TO_ORT_PATH(config.transducer.joiner_filename),
        sess_opts_);
