...
class SpeakerEmbeddingExtractor {
  constructor(configOrHandle) {
    if (configOrHandle && typeof configOrHandle === 'object' && configOrHandle.model !== undefined) {
      this.handle = addon.createSpeakerEmbeddingExtractor(configOrHandle);
  ...
  static async createAsync(config) {
    ...
    return new SpeakerEmbeddingExtractor(handle);
  }
  static create(config) {
    const handle = addon.createSpeakerEmbeddingExtractor(config);
    return new SpeakerEmbeddingExtractor(handle);
  }
  createStream() {
